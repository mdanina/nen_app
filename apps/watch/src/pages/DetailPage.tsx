...
export function DetailPage({ slug, expectedType, titles, favorites, toggleFavorite, navigate }: { slug: string; expectedType: ContentType; titles: WatchTitle[]; favorites: string[]; toggleFavorite: (id: string) => void; navigate: Navigate }) {
  ...
  return <div className="page detail-page">
    ...
    <div className="detail-grid">
      ...
      <section className="detail-section nen-rating"><p className="eyebrow">Рекомендация НЭН</p><h2>Возраст</h2><strong className="age-badge">{title.nenAgeRecommendation.minAge}{title.nenAgeRecommendation.maxAge ? `–${title.nenAgeRecommendation.maxAge}` : "+"} лет</strong><h3>Темы</h3><div className="tag-list">{title.themes.slice(0, 6).map((value) => <span key={value}>{value}</span>)}</div>{awards.length ? <><h3>Награды</h3><p>{awards.map((award) => `🏆 ${award}`).join(" · ")}</p></> : <><h3>Почему рекомендуем</h3><p className="recommendation-line">{compactRecommendation(title)}</p></>}</section>
      ...
      <section className="detail-section"><h2>Для разговора после просмотра</h2><ul>{title.discussionTopics.map((value) => <li key={value}>{value}</li>)}</ul><h3>Настроение</h3><div className="tag-list">{title.mood.map((value) => <span key={value}>{moodLabel(value)}</span>)}</div></section>
