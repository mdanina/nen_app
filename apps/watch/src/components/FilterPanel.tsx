...
const releaseFormLabels: Record<string, string> = {
  standalone: "Отдельное произведение",
  series: "Сериал",
};
...
export function FilterPanel({ filters, dictionary, onChange, onReset }: {
  ...
  return <section className="filters" aria-labelledby="filters-title">
    ...
    <fieldset><legend>Формат</legend><div className="segmented">
      {dictionary.contentFormats.map((format) => <button key={format} type="button" aria-pressed={filters.contentFormats.includes(format as never)} className={filters.contentFormats.includes(format as never) ? "selected" : ""} onClick={() => updateList("contentFormats", format)}>{formatLabels[format] ?? "Неизвестный формат"}</button>)}
    </div></fieldset>

    <fieldset><legend>Настроение</legend><div className="chips">{dictionary.moods.map((mood) => <button key={mood} type="button" className={filters.moods.includes(mood) ? "chip selected" : "chip"} aria-pressed={filters.moods.includes(mood)} onClick={() => updateList("moods", mood)}>{moodLabels[mood] ?? "Неизвестное настроение"}</button>)}</div></fieldset>
    ...
    <details><summary>Ещё фильтры</summary><div className="advanced-filters">
      ...
      {filters.contentType === "cartoon" && <>
        <fieldset><legend>Форма выпуска</legend>{dictionary.releaseForms.map((form) => <label className="check" key={form}><input type="checkbox" checked={filters.releaseForms.includes(form)} onChange={() => onChange({ ...filters, releaseForms: toggleFilterValue(filters.releaseForms, form) })} />{releaseFormLabels[form]}</label>)}</fieldset>
