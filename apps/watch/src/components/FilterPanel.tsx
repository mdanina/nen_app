...
import type { CatalogFilters, NumberRange } from "../domain/catalog/filters";
...
export function FilterPanel({ filters, dictionary, onChange, onReset }: {
  ...
  const updateList = (field: "moods" | "themes" | "excludedSensitiveTopics" | "countries" | "discussionPotentials" | "contentFormats", value: string) => {
    const current = filters[field] as string[];
    onChange({ ...filters, [field]: toggleFilterValue(current, value) } as CatalogFilters);
  };
  const updateNumber = (field: "durationMinutes" | "year", side: "min" | "max", event: ChangeEvent<HTMLInputElement>) => {
    const parsed = event.target.value === "" ? undefined : Number(event.target.value);
    const current = filters[field] ?? {};
    const next: NumberRange = { ...current, [side]: Number.isFinite(parsed) ? parsed : undefined };
    if (next.min === undefined && next.max === undefined) onChange({ ...filters, [field]: null } as CatalogFilters);
    else onChange({ ...filters, [field]: next } as CatalogFilters);
  };
  return <section className="filters" aria-labelledby="filters-title">
    ...
    <fieldset><legend>Продолжительность, минут</legend><div className="range-fields"><label>От<input type="number" min="1" max="360" value={filters.durationMinutes?.min ?? ""} onChange={(event) => updateNumber("durationMinutes", "min", event)} /></label><label>До<input type="number" min="1" max="360" value={filters.durationMinutes?.max ?? ""} onChange={(event) => updateNumber("durationMinutes", "max", event)} /></label></div></fieldset>
    ...
    <details><summary>Ещё фильтры</summary><div className="advanced-filters">
      ...
      <fieldset><legend>Год</legend><div className="range-fields"><label>От<input type="number" min="1888" value={filters.year?.min ?? ""} onChange={(event) => updateNumber("year", "min", event)} /></label><label>До<input type="number" min="1888" value={filters.year?.max ?? ""} onChange={(event) => updateNumber("year", "max", event)} /></label></div></fieldset>
      <fieldset><legend>Потенциал для обсуждения</legend>{dictionary.discussionPotentials.map((value) => <label className="check" key={value}><input type="checkbox" checked={filters.discussionPotentials.includes(value)} onChange={() => updateList("discussionPotentials", value)} />{value}</label>)}</fieldset>
      ...
      {filters.contentType === "cartoon" && <>
        ...
        <fieldset><legend>Длительность серии, минут</legend><div className="range-fields"><label>От<input type="number" min="1" max="180" value={filters.episodeDurationMinutes?.min ?? ""} onChange={(event) => { const value = event.target.value ? Number(event.target.value) : undefined; onChange({ ...filters, episodeDurationMinutes: { ...(filters.episodeDurationMinutes ?? {}), min: value } }); }} /></label><label>До<input type="number" min="1" max="180" value={filters.episodeDurationMinutes?.max ?? ""} onChange={(event) => { const value = event.target.value ? Number(event.target.value) : undefined; onChange({ ...filters, episodeDurationMinutes: { ...(filters.episodeDurationMinutes ?? {}), max: value } }); }} /></label></div></fieldset>
