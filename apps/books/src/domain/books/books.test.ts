...
describe("validateBooks", () => {
  ...
  it("разрешает внешнюю обложку другого издания после уверенного OCR-сопоставления произведения", () => { const result = validateBooks([book({ identificationStatus: "identified", isbn13: "9785000000001", bookFormats: ["повесть"], lifeSituations: [], emotionalStates: [], sensitiveTopicsReviewed: true, cover: { kind: "external", url: "https://publisher.example/book.jpg", rightsStatus: "external-display-only", isbn13: "9785000000002", temporary: true, assignmentMethod: "global_ocr_work_match", assignmentConfidence: 0.91, assignmentVerifiedAt: "2026-08-05" } })]); expect(result.issues).toHaveLength(0); expect(result.items).toHaveLength(1); });
