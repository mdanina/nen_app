...
export function BookFilters({ books, filters, update, clear, close }: { books: Book[]; filters: Filters; update: (next: Filters) => void; clear: () => void; close?: () => void }) {
  ...
  return <div className="filter-panel"><div className="filter-panel__head"><div><small>Фильтры применяются сразу</small><h2>Фильтры</h2></div>{close && <button className="icon-button" onClick={close} aria-label="Закрыть фильтры"><Icon name="close" /></button>}</div>
    ...
    <details className="more-filters"><summary>Другие фильтры</summary><ChipGroup title="Настроение" options={moods.map((v) => [v,v])} selected={filters.moods} toggle={(value) => toggle("moods",value)} /><fieldset><legend>Сложность языка</legend><div className="chips">{difficulties.map(([value,label]) => <button key={value} className={filters.difficulty === value ? "selected" : ""} aria-pressed={filters.difficulty === value} onClick={() => update({ ...filters, difficulty: filters.difficulty === value ? null : value })}>{label}</button>)}</div><small className="field-help">У некоторых книг сложность языка пока не указана.</small></fieldset></details>
